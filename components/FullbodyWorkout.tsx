...
import { FULLBODY_CIRCUIT, FULLBODY_ROUNDS } from '../constants';
...
const FullbodyWorkout: React.FC<FullbodyWorkoutProps> = ({ onComplete }) => {
  const [state, setState] = useState<WorkoutState>('NOT_STARTED');
  const [currentRound, setCurrentRound] = useState(1);
  const [currentExerciseIndex, setCurrentExerciseIndex] = useState(0);
  const [timer, setTimer] = useState(FULLBODY_CIRCUIT[0].duration);
  const [isRunning, setIsRunning] = useState(false);

  const currentExercise = FULLBODY_CIRCUIT[currentExerciseIndex];
  ...
  const advanceToNext = () => {
    ...
    if (nextExerciseIndex < FULLBODY_CIRCUIT.length) {
      setCurrentExerciseIndex(nextExerciseIndex);
      setTimer(FULLBODY_CIRCUIT[nextExerciseIndex].duration);
    } else {
      if (currentRound < FULLBODY_ROUNDS) {
        setCurrentRound(currentRound + 1);
        setCurrentExerciseIndex(0);
        setTimer(FULLBODY_CIRCUIT[0].duration);
      } else {
        setState('COMPLETED');
        setIsRunning(false);
      }
  ...
  if (state === 'RUNNING') {
    ...
    if (nextIndex < FULLBODY_CIRCUIT.length) {
        nextExerciseInfo = FULLBODY_CIRCUIT[nextIndex].name;
    } else if (currentRound < FULLBODY_ROUNDS) {
        nextExerciseInfo = FULLBODY_CIRCUIT[0].name;
    }
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center space-y-6">
      <h1 className="text-4xl font-bold text-white">Fullbody Workout</h1>
      <p className="text-slate-400">Total Rounds: {FULLBODY_ROUNDS}</p>

      {state === 'RUNNING' && (
        <>
          <p className="text-2xl text-white">Round: <span className="font-bold text-cyan-400">{currentRound}</span> / {FULLBODY_ROUNDS}</p>
